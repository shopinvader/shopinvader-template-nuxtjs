...
const getAllUrlKeys = async (index: string) => {
  ...
  return data?.aggregations?.url_keys?.buckets?.map((u: any) => u?.key || '') || []
}
export default cachedEventHandler(
  async () => {
    ...
    if (elasticsearch) {
      /** Elastic Provider */
      const urlProducts = (await getAllUrlKeys(elasticsearch?.indices?.products || '')) || []
      const urlCategories = (await getAllUrlKeys(elasticsearch?.indices?.categories || '')) || []
