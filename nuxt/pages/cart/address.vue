  ...
  <nuxt-layout name="checkout">
    ...
    <template #body>
      ...
      <div class="cart-addresses">
        <div class="cart-addresses__items">
          <h2>
        ...
        <div class="cart-addresses__method">
          <h2>{{ $t('cart.delivery.method.title') }}</h2>
          <cart-delivery-method></cart-delivery-method>
        </div>
        <div class="cart-addresses__submit">
          <button
...
import CartDeliveryMethod from '~/components/cart/CartDeliveryMethod.vue'
import CartAddressSelector from '~/components/cart/CartAddressSelector.vue'
...
export default defineNuxtComponent({
  ...
  components: {
    ...
    'cart-delivery-method': CartDeliveryMethod
  },
  async setup() {
    const i18n = useI18n()
...
<style lang="scss">
.cart-addresses {
  @apply flex w-full flex-wrap justify-stretch gap-6;
  &__items {
    @apply flex w-full flex-col gap-6 md:w-1/3 lg:w-1/2;
    .address {
      @apply flex-grow rounded-lg border border-primary p-4 shadow-none;
      .title {
        @apply m-0 flex items-center gap-4 text-xl font-bold uppercase leading-none;
      }
      .subtitle {
        @apply ml-4 pt-3;
      }
  ...
  &__method {
    @apply w-full bg-slate-100 p-8 md:w-2/3  lg:w-1/2;
  }
  &__submit {
    @apply flex w-full flex-grow justify-end gap-6 border py-4;
  }
}
