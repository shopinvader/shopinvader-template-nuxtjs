<template>
  <div class="btn-group">
    ...
    <button v-if="currentPage > 1" class="btn btn-sm" @click="changePage(currentPage - 1)">‹</button> 
    <button 
      v-for="page in pages"
      :key="page"
      class="btn btn-sm"
    ...
    <button v-if="currentPage < count" class="btn btn-sm" @click="changePage(currentPage + 1)">›</button> 
  </div>
</template>
<script lang="ts">
export default {
